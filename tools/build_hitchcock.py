#!/usr/bin/env python3
"""Build the "Hitchcock's Bible Names" (Roswell D. Hitchcock, public domain)
...
Source: https://ccel.org/ccel/hitchcock/bible_names.xml — ThML document,
entries laid out as <div2 title="A">...<glossary><term>Word</term>
<def><p>...</p></def>...</glossary></div2> for each letter A-Z (same shape
as Easton's ebd2.xml). No index/cross-reference div2 to skip here — every
lettered div2 is pure entries.
"""
...
SOURCE_URL = "https://ccel.org/ccel/hitchcock/bible_names.xml"
CACHE_FILE = Path(__file__).parent / ".cache" / "hitchcock" / "bible_names.xml"
OUT_DIR = Path(__file__).parent.parent / "biblia" / "modules" / "costumbres" / "hitchcock-bible-names"
...
def build():
    ...
    manifest = {
        'schemaVersion': 2,
        'id': 'hitchcock-bible-names',
        'type': 'costumbres',
        'name': "Hitchcock's Bible Names Dictionary",
        'abbreviation': 'Hitchcock, Nombres Bíblicos',
        'language': 'en',
        ...
        'navegacion': 'tematico',
        'description': (
            "An Interpreting Dictionary of Scripture Proper Names de Roswell "
            f"D. Hitchcock (1869), {len(entries)} entradas con el significado "
            "etimológico de nombres propios bíblicos de personas y lugares. "
            "Dominio público. El texto se muestra en inglés y se traduce "
            "automáticamente al español en tiempo de lectura (igual que "
            "Matthew Henry / Historia de la Iglesia)."
        ),
        'license': 'Dominio público (1869).',
        'sourceUrl': 'https://ccel.org/ccel/hitchcock/bible_names',
