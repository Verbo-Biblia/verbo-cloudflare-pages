...
**Bug de preámbulo conversacional (encontrado y corregido 2026-08-07, prefijo subió a `v3`):** Juan reportó que algunas traducciones aparecían con un párrafo pegado ARRIBA del texto traducido, del estilo "I'm ready to translate... please provide the text". Investigación:
- **Hallazgo #1 — el commit `e35be52` (fix del prompt a `v2`) nunca llegó a desplegarse.** Su timestamp de commit es `2026-08-07T10:18:06Z`, pero el último `wrangler deploy` de esa mañana fue a las `10:08:23Z` — 10 minutos ANTES. La verificación "contra el Worker ya desplegado" documentada en el párrafo de arriba se hizo con `wrangler dev` (local), no contra producción. El Worker en producción seguía corriendo una versión más vieja hasta el deploy de esta sesión. **Lección: `wrangler deploy` es un paso manual separado de `git push` — a diferencia de Cloudflare Pages (que despliega solo con cada push a `main`), el Worker NO se despliega solo. Después de cualquier cambio a `worker.js`, hay que correr `npx wrangler deploy` desde `cloudflare/api-bible-worker/` explícitamente, o el cambio queda solo en git sin efecto real en producción.**
- **Hallazgo #2 — causa raíz real, no era problema de caché.** Reproducido en vivo contra el Worker desplegado con `cached:false` (o sea, respuesta nueva del modelo, no una entrada vieja de KV): fragmentos cortos y sin contexto — una sola palabra ("soldier"), un término griego/hebreo ("agapao", "strateuomai") — disparaban al modelo a romper el personaje y responder con una negativa pidiendo "el texto real", en vez de traducir. Esto es exactamente la forma de las entradas del diccionario Strong, así que el bug probablemente se veía ahí más que en la prosa de Matthew Henry.
- **Fix aplicado — dos capas, ambas ya desplegadas:**
  1. **Prompt reforzado:** se agregó un párrafo explícito diciendo que el texto recibido a menudo es un fragmento corto (palabra suelta, término griego/hebreo, encabezado, referencia de capítulo/versículo) y que debe tratarse siempre como contenido real a traducir, nunca como prueba o falta de contexto — nunca pedir más contexto, nunca comentar, nunca negarse.
  2. **Red de seguridad server-side (nueva, en `worker.js`):** `looksLikeConversationalPreamble()` detecta patrones conocidos de negativa/aclaración (`PREAMBLE_PATTERNS`) al inicio de la respuesta. Si se detecta (o la respuesta viene vacía), el Worker reintenta la llamada a Anthropic una vez; si el reintento también falla, devuelve error 502 en vez de cachear o servir el preámbulo al cliente — el cliente ya tiene su propio fallback al texto original sin traducir ante un error de `/translate`, así que el usuario nunca ve el preámbulo, en el peor caso ve el texto sin traducir.
  3. Prefijo de caché subido de `v2` a `v3` (el prompt cambió).
  4. Limpieza: se borró del KV la única entrada contaminada con preámbulo que se generó durante las pruebas de esta sesión, y las 114 entradas huérfanas del prefijo `translate:v1:` (nunca leídas por el código desde que subió a `v2`, solo ocupaban espacio).
- Verificado tras el deploy: `soldier`→`soldado`, términos griegos/hebreos sueltos, encabezados cortos y prosa larga de Matthew Henry — todos traducen limpio sin preámbulo. Un texto adversarial diseñado a propósito para sonar como prueba (`"...test marker..."`) sigue empujando al modelo a negarse a veces, pero ahora la red de seguridad lo atrapa y devuelve 502 en vez de cachear/mostrar la negativa.
